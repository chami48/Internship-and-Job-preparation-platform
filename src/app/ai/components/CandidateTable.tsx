              ...
              >
                ...
                <td className="px-8 py-6">
                  <span className="text-sm font-black text-[#0F172A]">
                    {c.evalFailed ? "—" : `${c.score}/${c.maxScore}`}
                  </span>
                </td>
                <td className="px-8 py-6">
                  {c.evalFailed ? (
                    <span className="text-xs font-bold text-[#EA580C]">N/A</span>
                  ) : (
                    <div>
                ...
                <td className="px-8 py-6">
                  {c.evalFailed ? (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black uppercase tracking-widest bg-orange-100 border border-orange-300 text-orange-700">
                      ⚠ Error
                    </span>
                  ) : (
        ...
        {candidates.map((c) => (
          ...
          >
            ...
            <div className="flex gap-3 justify-between items-center">
              <span className="text-sm font-black text-[#0F172A]">
                {c.evalFailed ? "Eval Error" : `${c.score}/${c.maxScore} · ${c.percentage.toFixed(1)}%`}
              </span>
              <Link
                href={`/ai/candidate/${c.id}?jobId=${jobId}`}
